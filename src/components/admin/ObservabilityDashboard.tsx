      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400">
            {t("healthSubtitle", {
              minutes: Math.round(rates.windowMs / 60_000),
              time: new Date(metrics.updatedAt).toLocaleTimeString(),
            })}
          </p>
        </div>
        <button
          ...
          {alerts.map((alert) => (
            ...
            >
              ...
              <p className="mt-1 text-xs opacity-80">
                {alert.metric}: {formatRate(alert.currentRate)} (threshold{" "}
